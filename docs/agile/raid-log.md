...
| R3 | Hosting decision stays unresolved | Launch timing may slip late in the process | Decide host and DNS model during launch-readiness sprint |
...
| R5 | No version-control workflow is currently present | File-history tracking and rollback are weaker than ideal | Consider initializing git before final launch cycle |
...
| A2 | Squarespace can remain the registrar even if hosting changes | Confirm with final hosting plan |
...
| D4 | Hosting decision | Needed for DNS, deployment, and final checklist |
